...
def signal_handler(sig, frame):
    print('\nExiting...')
    visualise(data, g_max, g_min, close=False)
    sys.exit(0)
...
def visualise(data, g_max, g_min, close=True):
    #Data visualisation:
    df = pd.DataFrame([])
    df['y'] = data
    df['x'] = range(len(data))
...
while True:
    ...
    if validFrame:
        ...
        extra_count = 0
        if(len(data) > 1):
            extra_count, init_dir, control = getRepCount(data, init_dir, g_max, g_min, control)
